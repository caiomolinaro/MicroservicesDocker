...
public class ProductRepository : IProductRepository
{
    ...
    public async Task<IEnumerable<Product>> GetProductByCategoryAsync(string category)
    {
        FilterDefinition<Product> filter = Builders<Product>.Filter
            .Eq(product => product.Category, category);
    ...
    public async Task<IEnumerable<Product>> GetProductByNameAsybc(string name)
    {
        FilterDefinition<Product> filter = Builders<Product>.Filter
    ...
    {
        return await _context.Products.Find(products => true).ToListAsync();
    }

    public async Task<bool> UpdateProductAsync(Product product)
    {
        var updateResult = await _context.Products.ReplaceOneAsync(filter:
            productFilter => productFilter.Id == product.Id, replacement: product);
